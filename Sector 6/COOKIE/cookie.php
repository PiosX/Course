<?php
    ...
    if(isset($_GET['ref']))
    {
        ...
        {
            $_COOKIE['ref'] = $ref; //żeby odrazu sie zmienna pojawila bez 2krotnego odswiezania
...
<head>
    ...
    <title>Zmienna predefiniowana $_COOKIE</title>
</head>
<body>
        ...
        if ((isset($_POST['login']) && isset($_POST['haslo'])) || $_SESSION['zalogowany'] == 1)
        {
            if ((!empty($_POST['login']) && !empty($_POST['haslo'])) || $_SESSION['zalogowany'] == 1)
            {
                ...
                if (($login == "Armon" && $haslo == "abc") || $_SESSION['zalogowany'] == 1)
                {
                    ...
                    echo "Podałeś niepoprawny login lub hasło. Spróbuj ponownie <a href='cookie.php'>tutaj</a>";         
            }
            else
                echo "Nie podałeś loginu lub hasła. Spróbuj ponownie <a href='cookie.php'>tutaj</a>";    
        } 
        if($_SESSION['zalogowany'] == 0)       
        {
    ?>    
